...
public class VMWareDare2
{
    ...
    {
        final List<Set<Integer>> pairedRods = new ArrayList<>();
        
        for( final String s : pairs )
        {
            final String stArr[] = s.split( " " );
            
            Integer v = Integer.valueOf( stArr[0] );
            Integer u = Integer.valueOf( stArr[1] );
            
            boolean found = false;
            Set<Integer> setV = null;
            Set<Integer> setU = null;
            
            for( final Set<Integer> set : pairedRods )
            {
                if( set.contains( v ) )
                {
                    found = true;
            ...
            if( null != setU && null != setV )
            {
                if( !setU.equals( setV ))
                {
                    pairedRods.remove( setV );
                    pairedRods.remove( setU );
                    setV.addAll( setU );
                    pairedRods.add( setV );
                }
            }
