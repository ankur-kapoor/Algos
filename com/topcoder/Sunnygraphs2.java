...
public class Sunnygraphs2
{
    public long count( int[] a )
    {
        ...
        for( long i =0; i < 1L<<N; i++)
        {
            ...
            final List<Edge>[] newEdges = reconstructGraph( edges, b, a, N );
            
            if( isConnectedGraph( newEdges, N ))
            {
                res++;
            }
        }
    ...
    private List<Edge>[] reconstructGraph( final List<Edge>[] edges, final List<Integer> b, final int[]a, final int N )
    {
        final List<Edge>[] newEdges = new List[N+1];
        
        for( int i =0; i< edges.length; i++ )
        {
            newEdges[i] = new ArrayList<>(edges[i]);
        }
        
        newEdges[N] = new ArrayList<>();
        
        for( int i =0; i < b.size(); i++ )
        {
            int k = b.get( i );
            newEdges[k].remove( new Edge( k, a[k] ) );
            newEdges[a[k]].remove( new Edge(a[k], k) );
            
            newEdges[k].add( new Edge(k, N) );
            newEdges[N].add( new Edge(N, k) );
        }
        
        return newEdges;
    }
    
    private boolean isConnectedGraph( final List<Edge>[] edges, int N )
    {
        final long vMask = BFS( edges, N );
    ...
    private long BFS( final List<Edge>[] edges, int N )
    {
        final boolean[] visited = new boolean[edges.length];
        long visitingMask = 1L;
        long longMask = 1L;
    ...
    {
        final Sunnygraphs2 graph = new Sunnygraphs2();
