...
public class CustomDijkstraShortestPath
{
    public Map<Vertex<Integer>, Integer> shortestPath( Graph<Integer> graph, Vertex<Integer> sourceVertex )
    {
        ...
        while( !heap.empty())
        {
            ...
            for( Edge<Integer> edge : current.getEdges())
            {
                ...
                int newDist = Math.max( dist.get( current ), edge.getWeight() ) ;
    ...
    public static void main( String[] args )
    {
        ...
        CustomDijkstraShortestPath dsp = new CustomDijkstraShortestPath();
        
        int sum = 0;
        for( int i = 1; i < 5; i++ )
        {
            Vertex<Integer> sourceVertex = graph.getVertex(i);
            Map<Vertex<Integer>,Integer> distance = dsp.shortestPath(graph, sourceVertex);
          
            for( int j= i+1; j <= 5; j++)
            {
                sum += distance.get( graph.getVertex(j) );
            }
        }
        
        System.out.print( " Minimum sum = " + sum );
    }
}
